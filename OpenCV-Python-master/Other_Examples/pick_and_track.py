...
def my_mouse_callback(event,x,y,flags,param):
	...
	evente=event
	if event==cv2.CV_EVENT_LBUTTONDBLCLK:		# Here event is left mouse button double-clicked
		hsv=cv2.CreateImage(cv2.GetSize(frame),8,3)
		cv2.CvtColor(frame,hsv,cv2.CV_BGR2HSV)
		(h,s,v,i)=cv2.Get2D(hsv,y,x)
		(r,g,b,j)=cv2.Get2D(frame,y,x)
		print "x,y =",x,y
		print "hsv= ",cv2.Get2D(hsv,y,x)		# Gives you HSV at clicked point
		print "im= ",cv2.Get2D(frame,y,x) 	# Gives you RGB at clicked point
...
while(1):
	...
	if (leftmost-rightmost!=0) or (topmost-bottommost!=0):
		...
		if lastx!=0 and lasty!=0:
			...
			cv.Circle(imdraw,(posx,posy),25,(0,0,255),-1)
			print (posx,posy)
